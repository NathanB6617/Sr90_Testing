...
vector<double> GetMPV(TH1F* hist, string name);
...
// this is the code to call which just returns the MPV value of the distribution (taken from online - and modified)
vector<double> GetMPV(TH1F* hist, string filename)
{
    double fr[2];
    ...
    pllo[1]=5.0;             // MPV
    pllo[2]=1.0;             // Area
    pllo[3]=0.4;             // Gaussian Sigma

    // Parameter upper limits
    plhi[0]=5.0;             // width 
    plhi[1]=50.0;            // MPV
    plhi[2]=1000000.0;       // Area
    plhi[3]=10.0;             // Gaussian Sigma
    ...
    // Starting values based on the histogram
    sv[1] = hist->GetBinCenter(hist->GetMaximumBin()); // MP guess
    sv[0] = 0.08 * sv[1];                              // rough width guess
    sv[3] = 0.15 * sv[1];                              // rough sigma guess
    sv[2] = hist->Integral();

    // Parameter limits for ADC (gain about 45)
    // pllo[0]=0.5*45;      // 22.5
    // plhi[0]=5.0*45;      // 225
    ...
    TCanvas* c1 = new TCanvas("c1", fname.c_str(), 1200, 900);
    hist->Draw();
    fit->Draw("same");

    c1->SaveAs(Form("/home/lfhcal/Nathan/Sr90_Light_Yield_Testing/RAW_DATA/Run7/%s.png", fname.c_str()));
    
    return {MPV, fpe[1], chisqr, (double)ndf};
}
